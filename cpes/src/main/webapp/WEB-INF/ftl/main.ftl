	...
	<body class="">
		...
		<aside id="left-panel">
			...
			<div class="login-info">
				<span> <!-- User image size is adjusted inside CSS, it should stay as is --> 
					...
					<a href="javascript:void(0);" id="show-shortcut" data-action="toggleShortcut">
						<img src="${APP_PATH}/img/avatars/2233.png" alt="me" class="online" /> 
						<span>
							${userLogin.name} 
						</span>
			...
			<nav>
				...
				<ul>
				<#list ps.children as p>
					<li class="">
						<#if p.children?size != 0>
						<a style="cursor:pointer" title="${p.name}"><i class="fa-lg fa-fw ${p.icon }"></i> <span class="menu-item-parent">${p.name }</span></a>
							<ul>
							<#list p.children as cp>
								<li>
									<a href="${APP_PATH }/system/${cp.url}">${cp.name }</a>
								</li>
							</#list>
							</ul>
						<#else>
						<a href="${APP_PATH }/system/${p.url}"><i class="fa-lg fa-fw ${p.icon }"></i> <span class="menu-item-parent">${p.name }</span></a>
						</#if>
					</li>
				</#list>
				</ul>
		...
		<script>
			if (!window.jQuery.form) {
				document.write('<script src="${APP_PATH}/js/libs/jquery-form.min.js"><\/script>');
			}
		</script>
