...
<script type="text/javascript">
	$("#fprocessFile").change(function(event){
		var files = event.target.files;
		var file;
		if(files&&files.length>0)
			file = files[0];
		if(file&&file.size>2*1024*1024){
			layer.msg("上传的文件要小于2M",{time:1000, icon:5, shift:6});
			return ;
		} else {
			 $("#fprocessFile").ajaxSubmit({
	             type : "POST",
	             url  : "${APP_PATH}/system/process/insertProcDef.do?fusm=2",
	             ...
	             success : function(r){
	                 ...
	                 if ( r.success ) {
	                	 layer.msg("信息增加成功", {time:2000, icon:1},function(){
	                		 otable.draw();
		                     $("#processFile").reset();
	                	 });
	                 } else {
            ...
            "aoColumnDefs":[
	             {
	                 ...
	                 "mRender": function(data, type, full) {
	                	 return '<input type="checkbox" class="data-check" data-deployment-id="'+full.deploymentid+'" value="' + data + '" />';
	                 },
	                 "aTargets": [0]
	             },
                 {
                 ...
                 },
                 {
                     ...
                     "bSearchable": false,
                     "mRender": function(data, type, full) {
                         return "<a class='btn btn-primary' href='${APP_PATH}/system/process/showimg.htm?did="+full.id+"' data-toggle='modal' data-target='#remoteModal' >获取流程图</a>";
                     },
                     "aTargets": [5]
                 }
            ],
                    ...
                    },
                    {
                        ...
                        "fnClick": function ( nButton, oConfig, oFlash ) {
                        	...
                        	if ( processs.length == 0 ) {
					            layer.msg("请选择需要修改的流程信息", {time:1000, icon:5, shift:6});
                        	} else if ( processs.length > 1 ) {
                        		layer.msg("只能选择一条流程信息进行修改", {time:1000, icon:5, shift:6});
                        	} else {
                                var s = processs[0];
