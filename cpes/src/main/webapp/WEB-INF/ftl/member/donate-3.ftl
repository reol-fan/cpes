										...
										<div class="donate-area">
											...
											<div class="tab-content">
												<div id="home" class="tab-pane active fade in" role="tabpanel">
												    <form action="${APP_PATH}/member/uptEmail.do?id=${member.id}&mtype=${mtype}" method="post" id="certForm">
													<div class="donate-holder">
														<h3>认证邮箱</h3>
														<div class="form-area">
															<div class="input-area">
																<span><i class="fa fa-file-photo-o"></i></span>
																<input type="text" id="femail" name="email" value="${memberLogin.email}">
															</div>
														</div>
														<div class="cs-holder">
															<input type="submit" id="donateBtn3" value="提交实名认证申请">
														</div>
													</div>
													</form>
												</div>
...
<script>
	$(function(){
		$("#donateBtn3").click(function(){
			var email = $("#femail").val();
			var reg = /^[a-z\d]+(\.[a-z\d]+)*@([\da-z](-[\da-z])?)+(\.{1,2}[a-z]+)+$/;
			if(!reg.test(email)){
				layer.msg("邮件格式填写有误！", {time:2000, icon:5, shift:6});
				return false;
			}
			...
			$.ajax({
				type : "POST",
				data : {"email" : email},
				url : "${APP_PATH}/member/uptEmail.do",
				beforeSend : function(){
				...
				success : function(r){
	                ...
	                if ( r.success ) {
	                    window.location.href = "${APP_PATH}/member/donate-4.htm";
	                } else {
